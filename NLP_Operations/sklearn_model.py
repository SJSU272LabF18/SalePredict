...
test_sentence = "The quick brown fox clark jumps over the lazy dog. Carpe diem!"
...
## Handling unseen words in the test sentence
...
for i in range(len(test_sentence_tokens)):
    if(test_sentence_tokens[i].lower() not in stop_words && vectorizer.vocabulary_.get(test_sentence_tokens[i].lower())!= None)
    ## to lower and stop word remove
    
'''
    
print (test_sentence_tokens[4].lower())
print (vectorizer.vocabulary_.get(test_sentence_tokens[4].lower()))
